...
RUN apt-get update -y && apt-get install -y build-essential git nodejs npm wget \
...
RUN npm install --frozen-lockfile --prefix assets
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/twitter ./
